...
# ========== Preprocessing ==========
def preprocess_flattened(path, label):
    img = tf.io.read_file(path)
    img = tf.image.decode_jpeg(img, channels=3)
    img = tf.image.resize(img, img_size)
    img = tf.image.convert_image_dtype(img, tf.float32)
    img = tf.reshape(img, [-1])  # Flatten to 1D
    return img, label

def make_dataset(paths, labels):
    ds = tf.data.Dataset.from_tensor_slices((paths, labels))
    ds = ds.map(preprocess_flattened, num_parallel_calls=tf.data.AUTOTUNE)
    return ds.shuffle(1024).batch(batch_size).cache().prefetch(tf.data.AUTOTUNE)

train_ds = make_dataset(train_image_paths, train_labels)
...
callbacks = [
    keras.callbacks.ModelCheckpoint(
        "models/mlp_epoch-{epoch:02d}_val-acc-{val_accuracy:.4f}_val-loss-{val_loss:.4f}_f1score-{val_f1_metric:.4f}.keras",
...
def plot_roc(model, ds):
    ...
    plt.show()
    print(f"\n\U0001F539 AUC-ROC Score: {auc_score:.4f}")

# ========== Best Model Detection ==========
model_files = glob.glob("models/mlp_epoch-*.keras")
